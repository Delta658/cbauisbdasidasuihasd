...
config_tinyPortMapper(){
    echo -e "${Green}请输入搭建加速IP的配置信息！${Font}"
    read -p "请输入本地服务器端口（如25565）:" port1
    read -p "请输入加速服务器端口（如25565）:" port2
    read -p "请输入加速服务器IP（如209.222.114.1）:" tinyPortMapperip
}

start_tinyPortMapper(){
    echo -e "${Green}正在为您配置加速IP...${Font}"
...
echo -e "${Green}即将安装加速IP一键搭建...${Font}"
#获取最新版本号
...
KernelBit="$(getconf LONG_BIT)"
    ...
    echo -e "${Green}加速IP一键搭建安装成功！${Font}"
    else
    echo -e "${Green}加速IP一键搭建安装失败！${Font}"
    exit 1
    fi
#授可执行权
...
status_tinyPortMapper(){
    if [ -f /tinyPortMapper/tinymapper ]; then
    echo -e "${Green}检测到加速IP一键搭建已存在，并跳过安装步骤！${Font}"
        main_x
    else
        main_y
    fi
